...
mft_corr = mft / (1-C_t_ratio)*nans_mft #one data point is bad
mfb = mf_df.mf_b # mean lyman beta flux
mfb_corr = mfb / (1-C_b_ratio)
...
ax.errorbar(z_mf*1.001,mfa_corr,yerr=err_mfa, #F_ALPHA MEAN FLUX, UNCORRECTED
...
ax.errorbar(z_mf*1.000,mft_corr,yerr=err_mft, #F_TOTAL MEAN FLUX, UNCORRECTED
...
ax.errorbar(z_mf*0.998,mfb_corr,yerr=err_mfb, #F_BETA MEAN FLUX, UNCORRECTED
...
custom_lines[3] = ax.errorbar([],[],yerr=1,color='k',capsize=7,markersize=0,lw=1.5,capthick=3,elinewidth=4)
...
print("Figure saved here: {}".format(figname))
if show_plot:
    plt.show()
plt.clf()
